      ...
      for (const section of sections || []) {
        ...
        for (const p of passageRecs) {
          ...
          if (passageType === PassageTypeEnum.NOTE) {
            ...
            const newCont = `\\p ${p.attributes.reference}`;
            if (passCont) passCont += ` ${newCont}`;
            else passCont = newCont;
          ...
          for (let i = 0; i < versions; i++) {
            ...
            ingredients[destPath.substring(preLen)] = {
            ...
            verses.forEach((v) => {
              recs.push({
                antecedent: [`${timeFmt(v.start)} --> ${timeFmt(v.end)}`],
                anaphor: [`${book} ${v.label}`],
              });
            });
            if (recs.length > 0) {
              const timing = {
                ...
                documents: [
                  { scheme: 'timecode', docid: destName },
                  { scheme: 'u23003' },
                ],
                records: recs,
              };
              ...
              const namne = removeExtension(destName).name;
              const timingName = path.join(
                chapterPath,
                `${namne}-alignment.json`
              );
              const timingContent = JSON.stringify(timing, null, 2);
              await ipc?.write(timingName, timingContent);

              // add the timing file to the metadata file
