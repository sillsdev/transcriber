...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="para" mode="psg">
		<xsl:choose>
			...
			<xsl:when
				test="starts-with(@style, 'b') or starts-with(@style, 'q') or starts-with(@style, 'm')">
				<xsl:if test="local-name(preceding-sibling::*[1]) = 'para'">
					<xsl:apply-templates select="preceding-sibling::para[1]" mode="psg"/>
				</xsl:if>
			</xsl:when>
		...
		<xsl:choose>
			<xsl:when test="starts-with(@style,'s')">
				<xsl:if test="not(following-sibling::para[1][starts-with(@style,'s')])">
					<xsl:value-of
						select="count(preceding-sibling::para[starts-with(@style,'s')]/following-sibling::*[1][substring(@style,1,1)!='s'] | preceding-sibling::chapter[@number]/following-sibling::*[1][substring(@style,1,1)!='s']) + 1"/>
					<xsl:text>&#9;</xsl:text>
	...
	<xsl:template match="chapter[@number]">
		<xsl:if test="not(following-sibling::para[1][starts-with(@style,'s')])">
			<xsl:value-of select="count(preceding-sibling::para[starts-with(@style,'s')]/following-sibling::*[1][substring(@style,1,1)!='s'] | preceding-sibling::chapter[@number]/following-sibling::*[1][substring(@style,1,1)!='s']) + 1"/>
			<xsl:text>&#9;</xsl:text>
			<xsl:variable name="secs" select="preceding-sibling::para[starts-with(@style,'s')]"/>
			<xsl:variable name="secsCount" select="count($secs)"/>
			<xsl:value-of select="$secs[$secsCount]/text()"/>
			<xsl:text>&#9;</xsl:text>
			<xsl:text>&#9;</xsl:text>
			<xsl:text>&#9;</xsl:text>
			<xsl:text>&#9;</xsl:text>
